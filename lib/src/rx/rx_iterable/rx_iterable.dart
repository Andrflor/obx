...
extension RxIterableExt<T extends Iterable<E>, E> on Rx<T> {
  ...
  ///
  /// If this iterable only contains instances of [S], all operations
  ...
  ///
  /// If this [Rx<Iterable<T>>] only contains instances of [Iterable<S>], all operations
  /// will work correctly. If any operation tries to access an element
  /// that is not an instance of [S], the access will throw instead.
  ///
  ...
  ///
  /// The sets may contain fewer elements than the iterable,
  /// if the iterable contains an element more than once,
  /// or it contains one or more elements that are equal.
  /// The order of the elements in the set is not guaranteed to be the same
  /// as for the iterable.
  ...
  /// ```dart
  /// final planets = <int, String>{1: 'Mercury', 2: 'Venus', 3: 'Mars'};
  /// final keysRxList = Rx(planets.keys).toPipeList(growable: false);
  /// keysRxList.value; // [1, 2, 3]
  /// final valuesRxList =
  ///     Rx(planets.values).toPipeList(growable: false);
