...
extension RxMapExt<K, V> on Rx<Map<K, V>> {
  ...
  /// Cast this [Rx<Map<K, V>>] into an [Rx<Map<RK, RV>>]
	///
  /// If this map is already a [Rx<Map<RK, RV>>], it is returned unchanged.
  ///
  /// If this set contains only keys of type [RK] and values of type [RV],
  /// all read operations will work correctly.
  /// If any operation exposes a non-[RK] key or non-[RV] value,
  ...
  /// That means that you can do `mapWithStringKeys.cast<int,int>().remove("a")`
  /// successfully, even if it looks like it shouldn't have any effect.
  Rx<Map<RK, RV>> pipeCast<RK, RV>() => K == RK && V == RV ? this as Rx<Map<RK, RV>> :
			pipeMap((e) => e.cast<RK, RV>());
...
extension RxMapEntryExt<K, V> on RxMapEntry<K,V> {
